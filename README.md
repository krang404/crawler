...
Регистрация раннера производится с помощью плейбука /infra/ansible/registr_runner.yml, запуск которого нужно осуществлять определяя переменные YOUR_VM_IP и YOUR_TOKEN

После запуска контейнеров с Gitlab необходимо зайти на страницу
